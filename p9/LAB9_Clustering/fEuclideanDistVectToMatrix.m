...
%
% INPUT:
% - Vector: Column vector whose distance to the elements of x is going to
% be computed.
% - Matrix: Matrix containing the dataset.
% It has as many rows as features per sample and as many
% columns as samples.
%
% OUTPUT:
% -DistancesVector: a row vector with the distances between Vector and
% each element in Matrix
%
% EAlegre April2013
%% 1. Number of colums of the Matrix (containing the dataset)
NumberOfColums = size(Matrix,2);
%% 2. Replicating the rows of the Vector as many times as colums are in Matrix
% This way, it is easiest to operate
